...
#pragma GCC optimize("Ofast,no-stack-protector,fast-math,O3,-funroll-loops,-funroll-all-loops,-fpeel-loops")
#pragma GCC target("popcnt,avx2,lzcnt,abm,bmi,bmi2,fma")
...
void solve() {
    ...
    ll lb = 0, ub = 1e9+1; //Since an element is constrained between 1 and 1e9
    while(abs(lb - ub) > 1){
        ll mid = lb + (ub - lb) / 2;
        if(check(vec, mid)){
            lb = mid;
